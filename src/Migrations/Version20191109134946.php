 ...
 */
final class Version20191109134946 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__payment AS SELECT id, client_id, paid, amount, date FROM payment');
        $this->addSql('DROP TABLE payment');
        $this->addSql('CREATE TABLE payment (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, client_id INTEGER DEFAULT NULL, provider_id INTEGER DEFAULT NULL, paid BOOLEAN NOT NULL, amount DOUBLE PRECISION NOT NULL, date DATETIME NOT NULL, CONSTRAINT FK_6D28840D19EB6921 FOREIGN KEY (client_id) REFERENCES client (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_6D28840DA53A8AA FOREIGN KEY (provider_id) REFERENCES provider (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO payment (id, client_id, paid, amount, date) SELECT id, client_id, paid, amount, date FROM __temp__payment');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__provider AS SELECT id, user_id, name FROM provider');
        $this->addSql('DROP TABLE provider');
        $this->addSql('CREATE TABLE provider (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER DEFAULT NULL, name VARCHAR(255) NOT NULL COLLATE BINARY, CONSTRAINT FK_92C4739CA76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO provider (id, user_id, name) SELECT id, user_id, name FROM __temp__provider');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__payment AS SELECT id, client_id, paid, amount, date FROM payment');
        $this->addSql('DROP TABLE payment');
        $this->addSql('CREATE TABLE payment (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, client_id INTEGER DEFAULT NULL, paid BOOLEAN NOT NULL, amount DOUBLE PRECISION NOT NULL, date DATETIME NOT NULL)');
        $this->addSql('INSERT INTO payment (id, client_id, paid, amount, date) SELECT id, client_id, paid, amount, date FROM __temp__payment');
        $this->addSql('DROP TABLE __temp__payment');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_6D28840D19EB6921 ON payment (client_id)');
        $this->addSql('DROP INDEX UNIQ_92C4739CA76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__provider AS SELECT id, user_id, name FROM provider');
        $this->addSql('DROP TABLE provider');
        $this->addSql('CREATE TABLE provider (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER DEFAULT NULL, name VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO provider (id, user_id, name) SELECT id, user_id, name FROM __temp__provider');
        $this->addSql('DROP TABLE __temp__provider');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_92C4739CA76ED395 ON provider (user_id)');
        $this->addSql('DROP INDEX IDX_6117D13BA53A8AA');
